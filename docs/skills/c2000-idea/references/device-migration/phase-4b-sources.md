...
2. Read the full table row or function block at the anchor plus surrounding entries.
3. Follow referenced structs, enums, typedefs, macros if referenced.
...
if unavailable, stop and report: *"Cannot confidently fix `{symbol}` —
